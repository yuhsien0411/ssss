#!/usr/bin/env python3
"""
Modular Trading Bot with TICK support - Supports multiple exchanges
"""
...
from trading_bot_tick import TradingBot, TradingConfig
from exchanges import ExchangeFactory


def parse_arguments():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='Modular Trading Bot with TICK support - Supports multiple exchanges')
    ...
    parser.add_argument('--take-profit', type=Decimal, default=Decimal(0.02),
                        help='Take profit in percentage (default: 0.02 for 0.02%%)')
    parser.add_argument('--take-profit-tick', type=int, default=None,
                        help='Take profit in ticks (e.g., 5 for 5 ticks). If specified, overrides --take-profit')
    ...
    parser.add_argument('--grid-step-tick', type=int, default=None,
                        help='The minimum distance in ticks to the next close order price (e.g., 10 for 10 ticks). If specified, overrides --grid-step')
...
async def main():
    ...
    # Validate tick mode parameters
    if (args.take_profit_tick is not None and args.grid_step_tick is None) or \
       (args.take_profit_tick is None and args.grid_step_tick is not None):
        print("Error: --take-profit-tick and --grid-step-tick must be specified together for tick mode")
        sys.exit(1)
    ...
    config = TradingConfig(
        ...
        take_profit_tick=args.take_profit_tick,
        grid_step_tick=args.grid_step_tick
    )
